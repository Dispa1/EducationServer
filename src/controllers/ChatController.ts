...
export const createIndividualChat = async (req: Request, res: Response) => {
  try {
    const token = req.headers.authorization?.split(' ')[1];
    const decodedToken = jwt.verify(token as string, process.env.JWT_SECRET_KEY as string) as JwtPayload & { uid: string, username: string, fullname: string };
