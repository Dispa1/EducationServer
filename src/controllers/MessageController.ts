...
const getUsersInChat = async (chatId: number) => {
  try {
...
export const createMessage = async (req: Request, res: Response) => {
  try {
    ...
    const decodedToken = jwt.verify(token as string, process.env.JWT_SECRET_KEY as string) as JwtPayload & { uid: string };
...
export const deleteMessage = async (req: Request, res: Response) => {
  try {
    ...
    const decodedToken = jwt.verify(token as string, process.env.JWT_SECRET_KEY as string) as JwtPayload & { userId: string };
...
export const getAllMessagesFromChat = async (req: Request, res: Response) => {
  try {
    const token = req.headers.authorization?.split(' ')[1];
    jwt.verify(token as string, process.env.JWT_SECRET_KEY as string) as JwtPayload & { userId: string };
    ...
    const messageStatuses = await Promise.all(messages.map(async (message) => {
      const isMessageReadByAll = await Promise.all(chat.users.map(async (user: { id: string }) => {
...
export const createVoiceMessage = async (req: Request, res: Response) => {
  try {
    const token = req.headers.authorization?.split(' ')[1];
    ...
    upload.single('voiceMessage')(req, res, async (err) => {
      ...
      const voiceMessagePath = req.file?.path;
...
export const getLastMessageInChat = async (req: Request, res: Response) => {
  try {
    const token = req.headers.authorization?.split(' ')[1];
...
export const createMessageStatus = async (req: Request, res: Response) => {
  try {
    const token = req.headers.authorization?.split(' ')[1];
    const decodedToken = jwt.verify(token as string, process.env.JWT_SECRET_KEY as string) as JwtPayload & { uid: string };
...
export const updateMessageStatus = async (req: Request, res: Response) => {
  try {
    ...
    const updatedStatus = await UserMessageStatus.update(
      { isRead: true },
      { where: { userId: decodedToken.uid, messageId } }
    );
...
export const countUnreadMessagesInChat = async (req: Request, res: Response) => {
  try {
    const token = req.headers.authorization?.split(' ')[1];
    const decodedToken = jwt.verify(token as string, process.env.JWT_SECRET_KEY as string) as JwtPayload & { uid: string };
